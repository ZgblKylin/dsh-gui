...
- **SSH 连接（启动 + 安全隧道转发，VSCode Remote 风格）**：
  ...
  - **主机密钥 accept-new（按密钥类型比对）**：首次连接的远端会按 `~/.ssh/known_hosts` 记录；
    OpenSSH 每个主机每类密钥各记一行、SSH 握手只协商一个密钥类型，因此插件**只与同类型已记录密钥比对**——
    握手从 `ssh-rsa` 换成 `ssh-ed25519` 等属正常的“新类型首见”，会 accept-new 记录，不会误报拒绝；
    同一类型密钥**真变更**才拒绝连接（防 MITM）。拒绝时会给出可操作提示：
    远端主机、`known_hosts` 路径，以及 `ssh-keygen -R <主机>`（或删除对应行）后再连。
...
- **ssh2 传输与已知边界**：密码 / 密钥（含口令）通过 `ssh2` 原生认证（`keyboard-interactive` 也自动尝试）；`~/.ssh/config`
  ...
  仍**不支持** `ProxyJump`、证书主机密钥（`@cert-authority`）与哈希式 known_hosts 条目（未匹配时按首次连接接受）；
   主机密钥校验为**按密钥类型分列比对**（`ssh-rsa` / `ssh-ed25519` / `ecdsa-sha2-*` 互不当证据），
   同一类型密钥更换会被拒绝并在连接日志给出 `ssh-keygen -R <主机>` 的清除指引；

...
| Tab webviews track the window | `ui/app.js` relayouts every tab webview via `view_set_bounds` on `resize`/`ResizeObserver`; child webviews move with the window and hide/show on tab switch. Shell dialogs no longer hide them: the hamburger menu is a native popup (`show_config_menu`) and the connection/update/about dialogs open as **child-webview cards** (`dialog-*` via `src/dialogs.rs`) placed over the content area, so the harness content stays visible around them. |

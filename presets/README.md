...
2. **自备安装脚本**：每个预设目录携带自己的 `install.mjs`，负责把源文件
   ...
   共享工具链。例如 `anchored-standard` 在整目录复制后还会做幂等的
   落地补丁：检测 Windows 与 `rg`，把对应的最小提示注入第二轮
   `instruction-hint` 消息；把晋升后的 resident 目录中的 shell 换成
   standard 模式的平台 shell（Windows 用 `pwsh`，其他平台用 `bash`）；
   并在 Windows 上为晋升后的 agent scope 增加可逆的 `tools.restrict()`，
   让 `custom-bash` 的 `bash` 注册在运行时也被禁用（compaction 回退阶段
   会自动恢复，新晋升时再次禁用）。
...
└─ anchored-standard/              # 外置源：两阶段锚定标准模式（实验）
   ├─ install.mjs                  #   整目录复制 + 环境提示、晋升 shell 与运行时 bash 禁用补丁
...
- 预设源文件本身**必须保持直接可挂载**：源（内嵌文件或 submodule 检出）里的
  ...
  追加幂等的落地补丁（如 `anchored-standard` 注入 Windows/rg 提示、晋升 shell
  与运行时 bash 禁用），但源脱离补丁仍应可挂载。
...
| `anchored-standard` | Anchored Standard (experimental) | 两阶段预设：首次请求仅 `bash`/`str_replace_editor`（Minimal 对齐 prompt），首个持久 `tool/call` 或 `assistant/message` 后开放 minimal resident 工具集（standard 模式平台 shell + 发现工具），Windows 晋升后从 `custom-bash` 切换为 `pwsh`，其余经 `dev_tool_search` 按需解锁；安装脚本会注入 Windows/rg 第二轮提示、晋升 shell，并在 agent scope 运行时禁用晋升后的 `bash`（compaction 回退时恢复）；源来自 submodule [`xiaobright/dsh-anchored-standard`](https://github.com/xiaobright/dsh-anchored-standard) |

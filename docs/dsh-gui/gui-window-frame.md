...
are restored with small Win32 bridges. Each connection tab is hosted by its
own child webview (`src-tauri/src/views.rs`) placed exactly over the content
area below the title bar; the shell reports the logical rect of
`#harness-frame` (`view_set_bounds`) on every resize/DPI change because
tauri's ratio-based `auto_resize` cannot express a fixed-height top band.
...
| Tab webviews track the window | `ui/app.js` relayouts every tab webview via `view_set_bounds` on `resize`/`ResizeObserver`; child webviews move with the window and hide/show on tab switch. Shell dialogs no longer hide them: the hamburger menu is a native popup (`show_config_menu`) and the connection/update/about dialogs are **system-native windows** (pre-created hidden in `src/dialogs.rs`, owned by the main window, regular size with in-panel scrolling; content-driven resize is deferred until WebView2 settles to avoid freezing the process), so the harness content stays visible behind them. |

...
| Tab webviews track the window | `ui/app.js` relayouts every tab webview via `view_set_bounds` on `resize`/`ResizeObserver`; child webviews move with the window and hide/show on tab switch and while modal dialogs are open (`markModal`). |
